...
const highlights = [
  { icon: Music2, labelKey: "livePerformances" },
  { icon: Settings2, labelKey: "worldClassProduction" },
  { icon: UtensilsCrossed, labelKey: "foodAndBeverages" },
  { icon: ShieldCheck, labelKey: "secureEntry" },
]
...
export function AboutTab({ event }: AboutTabProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Description */}
      <div>
        <h2 className="text-base font-bold mb-3">{t("about.title")}</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {highlights.map(({ icon: Icon, labelKey }) => (
          ...
          >
            ...
            <p className="text-xs font-semibold text-center leading-tight">{t(`about.highlights.${labelKey}`)}</p>
          </div>
        ))}
      </div>
